...
class TwitterSinglePost extends Component
{
  constructor(props)
  {
    super(props);
  ...
  submitEditHandler(e)
  {
    ...
    this.setState({modalIsOpen: false});
    //this.props.deletetwposts(post_content_id)

    this.props.edittwpostque({
        post_content: post,
  ...
  render()
  {
    ...
    if (post.post_slot){
        var slot_index = this.props.twslots.findIndex(
            x => x.id==parseInt(this.props.post.post_slot));
        slot = this.props.twslots[slot_index].slot_time;
    }
...
export default connect(mapStateToProps,Actions)(TwitterSinglePost);